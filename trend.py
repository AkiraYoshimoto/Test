...
op=df[‘Open’].squeeze().values; hi=df[‘High’].squeeze().values
lo=df[‘Low’].squeeze().values;  cl=df[‘Close’].squeeze().values
...
lambda v,*: f’{v/1e6:.0f}M’ if v>=1e6 else f’{int(v/1e3)}K’))
ax3=axes[2]
hist=df[‘MHST’].values
...
st.set_page_config(page_title=‘Trend Signal’,page_icon=‘📈’,
